...
```{r, echo = FALSE}
# feature server url
bts_url='https://services2.arcgis.com/C8EMgrsFcRFL6LrL/arcgis/rest/services/Bottom_Trawl_Survey_Strata/FeatureServer'

# metadata
bts_meta = arcgisutils::fetch_layer_metadata(url = bts_url, token = token)
```
...
bts_fields = purrr::map(.x = 1:nrow(bts_layers ), .f = function(.x) {
  ...
  # open layer
  bts_layer = arcgislayers::arc_open(url = paste0(bts_url, '/', bts_layers$id[.x]), token = token)
