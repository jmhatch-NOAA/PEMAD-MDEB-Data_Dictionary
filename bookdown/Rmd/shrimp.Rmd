...
```{r, echo = FALSE}
# feature server url
shrimp_url='https://services2.arcgis.com/C8EMgrsFcRFL6LrL/arcgis/rest/services/Northern_Shrimp_Survey_Strata/FeatureServer'

# metadata
shrimp_meta = arcgisutils::fetch_layer_metadata(url = shrimp_url, token = token)
```
...
shrimp_fields = purrr::map(.x = 1:nrow(shrimp_layers ), .f = function(.x) {
  ...
  # open layer
  shrimp_layer = arcgislayers::arc_open(url = paste0(shrimp_url, '/', shrimp_layers$id[.x]), token = token)
