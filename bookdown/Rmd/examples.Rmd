...
You can also select a subset of columns or fields to return, just populate the `fields` argument in `arc_read()`. It is useful to note that `arc_read()` is just a wrapper around the functions `arc_open()` and `arc_select()`. Sometimes using `arc_open()` and `arc_select()` can be faster than `arc_read()` for some reason.
